...
def select_kernel(no_dims):
     ...
     kernel .
     """
     kernel_list = ['StdPeriodic','RBF','Matern32']
#     kernel_list   = ['RBF']
     kernel_number = np.random.randint(0,len(kernel_list))
     ...
     if kernel_name != 'MLP':
         ...
         wv = np.abs(np.random.randn())
         bv = np.abs(np.random.randn())
         kernel = getattr(GPy.kern,kernel_list[kernel_number])(input_dim=no_dims,\
...
def unit_data(data):
    numer       = data - np.amin(data)
    denom       = np.amax(data) - np.amin(data)
    data_unit   = numer / denom
    try:
        assert np.amax(data_unit) < 1.0000001 and np.amin(data_unit) > -0.00000001
    except:
        print('Here lies the problem ' , denom)
    return data_unit
...
def save_data(input_data,nrmse,kernel_params,post_mean,post_var,parameters):
    ...
    time_pred = input_data['val'][0:no_points,no_dims-1].flatten()
    time_all  = input_data['val'][:,no_dims-1].flatten()
    cov_diag  = post_var.flatten()
    kernel_params = kernel_params.flatten()
    
    d  = dict(Time_pred = time_pred, Time = time_all,Predicted = predicted, Actual = actual, Diag_cov = cov_diag,Kernel_params = kernel_params, Kernels = kernel_name, NRMSE = nrmse, Variance = var, Length_scale = ls)
    df = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in d.items() ]))
    PATH = '../Data/' +model+ '/data_layer_/' + option 
    file   = PATH + '/'+str(layer_no)+'_' + str(no_points) +'.csv'
    os.makedirs(PATH,exist_ok = True)
    columns = ['Inputpred', 'Inputall','Predicted', 'Actual', 'Diag_cov' ,'Kernel_params', 'Kernels', 'NRMSE', 'Variance', 'Length_scale']
    df.to_csv(file, columns= columns)
def save_data2d(input_data,nrmse,kernel_params,post_mean,post_var,parameters):
    no_points     = parameters[0]
    option        = parameters[1]
    no_dims       = parameters[2]
    model         = parameters[3]
    ...
    cov_diag      = post_var.flatten()
    kernel_params = kernel_params.flatten()
    input_pred    = input_data['val'][0:no_points,no_dims-2:]
    input_all     = input_data['val'][:,no_dims-2:]
    data          = [input_pred[:,0:1].flatten(),input_pred[:,1:2].flatten(),input_all[:,0:1].flatten(),input_all[:,1:2].flatten()]
    
    d    = dict(Input1pred = data[0], Input2pred = data[1], Input1all = data[2], Input2all = data[3],Predicted = predicted, Actual = actual, Diag_cov = cov_diag,Kernel_params = kernel_params, Kernels = kernel_name, NRMSE = nrmse, Variance = var, Length_scale = ls)
    df   = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in d.items() ]))
    # To do, sort data frame so that cols are *inputs* , actual, predicted, NRMSE, *rest does not matter* 
    PATH   = '../Data/' +model+ '/data_layer_/' + option 
    file   = PATH + '/'+str(layer_no)+'_' + str(no_points) +'.csv'
    os.makedirs(PATH,exist_ok = True)
    columns = ['Input1pred', 'Input2pred', 'Input1all', 'Input2all','Predicted', 'Actual', 'Diag_cov' ,'Kernel_params', 'Kernels', 'NRMSE', 'Variance', 'Length_scale']
    df.to_csv(file, columns= columns)
def plotting(data,nrmse,parameters,full_cov):
   ...
   """
    ...
    plt.plot(Xtest[no_points:,no_dims-1].flatten(), Ytest[no_points:].flatten(), 'b.', label='test data')
    plt.fill_between(Xtest[:,no_dims-1].flatten(), mu-2*s, mu+2*s, color="#C0C0C0", label = 'mu +/- 2sd')
    plt.plot(Xtest[:,no_dims-1].flatten(), mu, 'w-', lw=1, label = 'Prediction')

    plt.legend(loc='best',prop={'size':4})
    name_of_plot = 'Layer ' + str(layer_no) + " with NRMSE  " + str(nrmse)
    ...
    plt.imshow(full_cov)
    plt.colorbar()
    name         = 'Covariance_in_layer ' + str(layer_no)
...
def plotting2d(data,nrmse,parameters,full_cov):
    """For plotting the graphs to verify the confidence in the prediction of
       our training data. Saves plots in ../../Plots. Takes the model, m
       and uses GPys inbuilt plotting. 
       Inputs:
   ...
   """
    ...
    if no_dims == 3:
        ...
        plt.plot(Xtrain[:,0].flatten(),Ytrain.flatten(), 'r.',label = 'training data')

    plt.plot(Xtest[no_points:,no_dims-2].flatten(), Ytest[no_points:].flatten(), 'b.', label='test data')
    plt.fill_between(Xtest[:,no_dims-2].flatten(), mu-2*s, mu+2*s, color="#C0C0C0", label = 'mu +/- 2sd')
    plt.plot(Xtest[:,no_dims-2].flatten(), mu, 'w-', lw=1, label = 'Prediction')
